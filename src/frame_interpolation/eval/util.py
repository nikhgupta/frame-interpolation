...
def _recursive_generator(
    ...
    block_width: Optional[int], block_height: Optional[int]
    ...
    if num_recursions == 0:
        ...
        image_batch_1 = np.expand_dims(frame1, axis=0)
        image_batch_2 = np.expand_dims(frame2, axis=0)

        # Invoke the model once.
        mid_frame = None
        if block_height is not None and block_width is not None:
            block_shape = [block_height, block_width]
            patches_1 = image_to_patches(image_batch_1, block_shape)
            patches_2 = image_to_patches(image_batch_2, block_shape)
            ...
            for image_1, image_2 in zip(patches_1, patches_2):
                ...
                mid_patch = interpolator.interpolate(
                    image_batch_1, image_batch_2, time)
                output_patches.append(mid_patch)
            output_patches = np.concatenate(output_patches, axis=0)
            mid_frame = patches_to_image(output_patches, block_shape)[0]
        else:
            mid_frame = interpolator.interpolate(
                image_batch_1, image_batch_2, time)[0]

        yield from _recursive_generator(frame1, mid_frame, num_recursions - 1, interpolator, block_width, block_height)
        yield from _recursive_generator(mid_frame, frame2, num_recursions - 1, interpolator, block_width, block_height)


def interpolate_recursively_from_files(
        frames: List[str], times_to_interpolate: int,
        interpolator: interpolator_lib.Interpolator,
        block_width: Optional[int] = None, block_height: Optional[int] = None) -> Iterable[np.ndarray]:
